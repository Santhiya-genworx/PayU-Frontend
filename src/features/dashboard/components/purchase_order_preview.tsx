...
export default function PurchaseOrderPreviewModal({file, onClose, onUpdate}: { file: ExtractedFile; onClose: () => void; onUpdate: (updatedFile: ExtractedFile) => void;}) {
  ...
  const numericFields = new Set<string>([
    "quantity", "unit_price", "total_price", "total_amount",
  ]);
  ...
  const numericItemFields: ReadonlySet<keyof OrderedItem> = new Set([
  ...
  const setItem = (i: number, key: keyof OrderedItem, val: string) => {
    setForm((prev) => {
      ...
      const item = items[i] as Record<keyof OrderedItem, unknown>;
        ...
        <div className="flex flex-1 overflow-hidden divide-x divide-gray-100">
          ...
          <div className="w-1/2 overflow-y-auto p-5 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="GL Code"        value={form.gl_code}       onChange={(v) => set("gl_code", v)} />
              <Field label="Ordered Date"   value={form.ordered_date}  type="date"   onChange={(v) => set("ordered_date", v)} />
          ...
          <div className="w-1/2 flex flex-col overflow-hidden bg-gray-50">
            ...
            {isImage ? (
              ...
              <iframe src={URL.createObjectURL(file.file)} className="flex-1 w-full border-0" title="PO File Preview" />
            )}
          </div>
        </div>
